...
### 1. ERD
#### | 데이터베이스 엔티티 관계 다이어그램
<img width="700" height="547" alt="image" src="https://github.com/user-attachments/assets/1300e5ff-7b7f-4435-a585-ebe0286c2c4b" />
...
- **기술적 선택 이유**
  ...
  - `ApplicationEventPublisher` 의존성 분리 <p> 결제 도메인과 데이터 전송 도메인의 의존성을 분리해서 사용자에게 응답 솔도가 빠르게 했고, 서버에 에러가나도 비즈니스로직과 알림 전송 로직이 독립적으로 유지되서 서로 영향이 없습니다.
    ...
    ```
- 기술블로그 정리 : [Mock API 활용 및 트랜잭션 고민](https://velog.io/@onnuri1226/아키텍쳐-결제는-성공했는데-통계-서버가-죽어서-내-결제가-취소-Mock-API-활용-및-트랜잭션-고민)

### 3. 실시간 인기 메뉴 조회
- **문제 상황** :
  - 주문 수가 많아진다면, `GROUP BY`와 `COUNT`로 집계해 인기 순위를 산출할 경우, DB에 연산 부하가 발생하고 응답 속도가 저하되는 이슈를 생각해보았습니다
- **분석 내용** :
  - RDBMS (SQL) : <p> 대용량의 정보를 쓰기에 사용량이 많은 환경에서는 읽기 성능이 떨어지며 빠른 실시간 처리에 한계가 있음
  - NoSQL (Redis) : <p> 인메모리 DB에서는 메모리에서 직접 데이터를 처리해 빠른 속도를 보장해 실시간 반영 가능
- **기술적 선택 이유** :
  - Redis Zset (Sorted Set) 도입 <p> 인기 메뉴는 약간의 오차가 허용되는 통계성 데이터이므로 모든 데이터를 DB에서 읽어오기보다 캐시(Cache)를 활용하여 사용자 경험을 개선하는 것이 효율적이라 판단
    - RDBMS 쿼리 방식은 데이터가 변경될 때마다 인덱스 재구성 및 짐계 과정을 거쳐야 함으로, 초 단위의 **실시간** 순위 반영을 구현하기에 성능 한계가 있음
    - Zset 은 데이터를 넣는 동시에 `Score`을 기준으로 정렬된 상태를 유지하므로 별도의 정렬 쿼리가 필요없음
    - 수백개의 메뉴가 있더라도 `incrementScore` 연산과 순위 조회 작업이 매우 빠름
    - 여러사용자가 동시에 주문해 카운트를 올릴 때 발생할 수 있는 동시성 이슈를 Redis 내부 엔진이 원자적으로 처리해 데이터 정합성유지가 유리
    - Time Window 개념을 적용한 `고인물` 데이터 삭제
      ```java
      // 통합 결과 키가 영구적으로 남지 않도록 10분 뒤 자동 만료 TTL
      stringRedisTemplate.expire(destKey, java.time.Duration.ofMinutes(10));
      ```

- **기술블로그 정리** : [실시간 인기 메뉴 랭킹 만들기 : Redis Zset 활용](https://velog.io/@onnuri1226/%EB%8D%B0%EC%9D%B4%ED%84%B0%EB%B2%A0%EC%9D%B4%EC%8A%A4-%EC%8B%A4%EC%8B%9C%EA%B0%84-%EC%9D%B8%EA%B8%B0-%EB%A9%94%EB%89%B4-%EB%9E%AD%ED%82%B9-%EB%A7%8C%EB%93%A4%EA%B8%B0-Redis-Zset-%ED%99%9C%EC%9A%A9)
